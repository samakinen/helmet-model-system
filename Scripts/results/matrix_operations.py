...
def set_emmebank_matrices(emme_project, matrices, fname):
    no_mtx = len(matrices.keys())
    print "Reads {} matrices to Emme-project".format(no_mtx)
    while True:
        try:
            mtx_id = int(input("Matrix id start import from:"))
            break
        except ValueError:
    ...
    for mtx in matrices:
        ...
        emme_project.create_matrix(
            matrix_id=idx,
            matrix_name=fname,
            matrix_description=fname + ": " + mtx_label,
            default_value=0,
            overwrite=True
            )
...
def transit_mode_matrices(emme_project, scenarios, mtxdict):
    for name in mtxdict:
        ...
        emme_project.strategy_analysis(transit_strategy_spec, scenario, num_processors ="max")
        print "Ready {} {}".format(name, scenario)

def transfer_matrices(emme_project, scenarios, mtxdict):
    for name in mtxdict:
        tp = mtxdict[name]['scenario']
        emmebank = emme_project.modeller.emmebank
        scenario = emmebank.scenario(scenarios[tp])
...
def aggregate_matrix(aggregation_data, mtxs, result_path, zone_numbers, fname):
    ...
    if aggregation_data is not None:
        ...
        for mode_keys in mtxs.keys():
            ...
            fname0 = "{}_{}.txt".format(fname, str(mode_keys))
            print_aggregated_data(mtx_agg, result_path, fname0)
